...
def handle_client_connection(connection, address, fp_instance_thread):
    current_thread_name = threading.current_thread().name
    logging.info(f"Memproses koneksi dari {address} menggunakan thread {current_thread_name}")
    ...
    try:
        ...
        if command_str:
            ...
            try:
                hasil = fp_instance_thread.proses_string(command_str)
        ...
        logging.info(f"Tugas worker untuk {address} selesai dengan status: [{client_task_status}] oleh {current_thread_name}")
    ...
    def _handle_signal(self, signum, frame):
        ...
        if self.my_socket:
            try:
                ...
                logging.debug(f"Error saat menutup socket di signal handler: {e}")
    ...
    def run(self):
        ...
        if not os.path.exists(SERVER_STORAGE_DIRECTORY):
            try:
                ...
                logging.error(f"Gagal membuat direktori '{SERVER_STORAGE_DIRECTORY}': {e}")

        logging.warning(f"Server (ThreadPool) memulai pada {self.ipinfo} dengan {self.max_workers} pekerja")
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(
                ...
                thread_name_prefix="ClientHandlerThread"
            ) as executor:
                self.executor = executor
                logging.info(f"ThreadPoolExecutor dimulai dengan max_workers={self.max_workers}")
                ...
                while not self._shutdown_request.is_set():
                    try:
                        ...
                        if self._shutdown_request.is_set():
                            logging.info("Permintaan shutdown diterima, koneksi baru tidak diproses.")
                            connection.close()
                            break
                        
                        logging.info(f"Menerima koneksi dari {client_address}")
                        executor.submit(handle_client_connection, connection, client_address, self.fp_instance)
                    except socket.timeout:
                        continue
                    except OSError as e:
                        ...
                        if self._shutdown_request.is_set():
                            logging.info(f"Exception saat accept setelah shutdown: {e_accept}")
                        else:
            ...
            logging.error(f"Error kritis dalam lingkup utama ThreadPoolExecutor: {e_main_executor_scope}", exc_info=True)
        finally:
            logging.warning("Blok 'finally' Server.run() dieksekusi.")
            if self.executor:
                logging.info("Memulai shutdown ThreadPoolExecutor...")
                self.executor.shutdown(wait=True)
                logging.info("ThreadPoolExecutor telah di-shutdown.")
            
            if self.my_socket:
                logging.info("Menutup socket server utama di finally (jika masih terbuka)...")
                try:
                    self.my_socket.close()
                    self.my_socket = None
                    logging.warning("Socket server utama telah ditutup.")
                except OSError as e:
                    logging.debug(f"Error saat menutup socket server utama di finally: {e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="File Server dengan ThreadPoolExecutor")
    parser.add_argument('--port', type=int, default=6666, help='Port untuk listen')
    parser.add_argument('--workers', type=int, default=5, help='Jumlah worker thread')
